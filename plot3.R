plot3 <- function(datafile, pngfile) {
    ...
    plot(strptime(paste(tbl_hh1$Date, tbl_hh1$Time),"%d/%m/%Y %H:%M:%S"), tbl_hh1$Sub_metering_2, type="l", ylab="Energy sub metering", xlab="")
    plot(strptime(paste(tbl_hh1$Date, tbl_hh1$Time),"%d/%m/%Y %H:%M:%S"), tbl_hh1$Sub_metering_3, type="l", ylab="Energy sub metering", xlab="")
